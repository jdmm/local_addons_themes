...
class index(main.Home):
    ...
    def index(self):
        velocidad=60
        return http.request.website.render('website_index.index', {'edit':False
        })
    @http.route('/left-sidebar', auth='public', website=True)
    def left_sidebar(self):
        velocidad=60
        return http.request.website.render('website_index.left_sidebar', {
        })
    @http.route('/right-sidebar', auth='public', website=True)
    def right_sidebar(self):
        velocidad=60
        return http.request.website.render('website_index.index', {
        })
    @http.route('/no-sidebar', auth='public', website=True)
    def no_sidebar(self):
        velocidad=60
        return http.request.website.render('website_index.index', {
        })

class WebsiteBlog_inherit(main_blog.WebsiteBlog):
    
    def __init__(self):
        super(WebsiteBlog_inherit, self).__init__() 
